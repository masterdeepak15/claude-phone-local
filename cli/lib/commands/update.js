...
async function fetchLatestRelease() {
  try {
    const response = await fetch(
      'https://api.github.com/repos/masterdeepak15/claude-phone-local/releases/latest',
      {
        headers: {
...
function showManualInstructions(release) {
  ...
  console.log(chalk.bold('   npm install -g claude-phone-local\n'));
...
export async function updateCommand() {
  ...
  if (isGitRepo(projectRoot)) {
    ...
    try {
      ...
      console.log(chalk.gray('Visit https://github.com/masterdeepak15/claude-phone-local for manual update\n'));
    }
  }
}
