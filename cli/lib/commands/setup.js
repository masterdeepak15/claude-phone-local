...
import { ensureVoiceModel, KNOWN_VOICES } from '../voice-downloader.js';
...
function createDefaultConfig() {
  return {
    version: '1.0.0',
    api: {
      mode: 'local', // 'local' (no API keys, offline STT/TTS) or 'cloud' (ElevenLabs + OpenAI)
      local: {
    ...
    paths: {
      ...
      sttLocal: path.join(getProjectRoot(), 'stt-local'),
      ttsLocal: path.join(getProjectRoot(), 'tts-local')
    }
  };
}
...
async function setupAPIKeys(config) {
  // ---- Local vs Cloud STT/TTS ----
  const { mode } = await inquirer.prompt([
    {
      type: 'list',
      name: 'mode',
      message: 'How should speech-to-text and text-to-speech work?',
      choices: [
        {
          name: 'Local (offline, no API keys, no cost — faster-whisper + Piper)',
          value: 'local'
        },
        {
          name: 'Cloud (ElevenLabs + OpenAI — requires paid API keys)',
          value: 'cloud'
        }
      ],
      default: config.api.mode || 'local'
    }
  ]);
  ...
  if (mode === 'local') {
    const localAnswers = await inquirer.prompt([
      {
        type: 'input',
        name: 'voice',
        message: 'Piper voice to use (default: en_US-lessac-medium):',
        default: config.api.local?.voice || 'en_US-lessac-medium'
      },
      {
        type: 'list',
        name: 'whisperModel',
        message: 'faster-whisper model size (bigger = more accurate, slower):',
        choices: ['tiny', 'base', 'small', 'medium'],
        default: config.api.local?.whisperModel || 'small'
      }
    ]);

    config.api.local = {
      sttUrl: config.api.local?.sttUrl || 'http://stt-local:9001',
      ttsUrl: config.api.local?.ttsUrl || 'http://tts-local:9002',
      voice: localAnswers.voice,
      whisperModel: localAnswers.whisperModel
    };

    // No cloud keys needed — leave them blank so nothing downstream requires them.
    config.api.elevenlabs = { apiKey: '', defaultVoiceId: '', validated: false };
    config.api.openai = { apiKey: '', validated: false };

    // Auto-download the Piper voice model so setup is zero-manual-download
    // for any of the common voices. Unknown/custom voice names are skipped
    // with a note — those still need to be placed manually.
    const voicesDir = path.join(config.paths.ttsLocal, 'voices');
    const dlSpinner = ora(`Checking Piper voice "${localAnswers.voice}"...`).start();
    try {
      const result = await ensureVoiceModel(localAnswers.voice, voicesDir);
      if (result.downloaded) {
        dlSpinner.succeed(chalk.green(`Downloaded voice "${localAnswers.voice}"`));
      } else if (result.skipped) {
        dlSpinner.succeed(chalk.green(`Voice "${localAnswers.voice}" already present`));
      } else {
    ...
    console.log(chalk.green('\n✓ Local mode selected — no API keys required.'));
    console.log(chalk.gray('  Piper (TTS) and faster-whisper (STT) will run as local Docker containers.\n'));

    return config;
  }

  // ---- Cloud mode: original ElevenLabs + OpenAI flow ----
  // ElevenLabs API Key
