...
async function runVoiceServerChecks(config, isPiSplit) {
  ...
  if (config.api && config.api.mode === 'local') {
    // Local mode: check the faster-whisper and Piper sidecar containers instead of cloud APIs
    const sttSpinner = ora('Checking local STT (faster-whisper)...').start();
    try {
      // Doctor runs on the host, not inside the Docker network, so it must
      // use the localhost-published port (see docker-compose.yml), not the
      // internal service-name URL that voice-app uses.
      await axios.get('http://127.0.0.1:9001/health', { timeout: 3000 });
      sttSpinner.succeed(chalk.green('Local STT (stt-local) is reachable'));
      passedCount++;
      checks.push({ name: 'Local STT', passed: true });
    } catch (error) {
      sttSpinner.fail(chalk.red(`Local STT unreachable: ${error.message}`));
      console.log(chalk.gray('  → Check "docker compose logs stt-local"\n'));
      checks.push({ name: 'Local STT', passed: false });
    }

    const ttsSpinner = ora('Checking local TTS (Piper)...').start();
    try {
      await axios.get('http://127.0.0.1:9002/health', { timeout: 3000 });
      ttsSpinner.succeed(chalk.green('Local TTS (tts-local) is reachable'));
      passedCount++;
      checks.push({ name: 'Local TTS', passed: true });
    } catch (error) {
      ttsSpinner.fail(chalk.red(`Local TTS unreachable: ${error.message}`));
      console.log(chalk.gray('  → Check "docker compose logs tts-local"\n'));
      checks.push({ name: 'Local TTS', passed: false });
    }
  } else {
