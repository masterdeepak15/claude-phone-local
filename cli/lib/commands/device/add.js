...
export async function deviceAddCommand() {
  ...
  const answers = await inquirer.prompt([
    ...
    },
    {
      type: 'input',
      name: 'voiceId',
      message: config.api.mode === 'local'
        ? 'Piper voice for this device (e.g. en_US-lessac-medium):'
        : 'ElevenLabs voice ID:',
      default: config.api.mode === 'local'
        ? (config.api.local?.voice || 'en_US-lessac-medium')
        : (config.api.elevenlabs.defaultVoiceId || ''),
      validate: (input) => {
        if (!input || input.trim() === '') {
          return 'Voice cannot be empty';
        }
        return true;
      }
    },
  ...
  // Validate voice ID with ElevenLabs API — only relevant in cloud mode.
  // In local mode, the "voice" is just a Piper model name, nothing to validate remotely.
  let voiceResult = { valid: true, name: answers.voiceId };
  if (config.api.mode !== 'local') {
    const spinner = ora('Validating voice ID with ElevenLabs...').start();
    voiceResult = await validateVoiceId(config.api.elevenlabs.apiKey, answers.voiceId);
